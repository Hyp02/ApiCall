-- 创建库
create database if not exists apicall;
...
-- 接口信息
create table if not exists apicall.`interface_info`
(
...
-- 用户调用接口关系表
create table if not exists apicall.`user_interface_info`
(
    `id`              bigint                             not null auto_increment comment '主键' primary key,
...
insert into apicall.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                      `method`, `userId`)
